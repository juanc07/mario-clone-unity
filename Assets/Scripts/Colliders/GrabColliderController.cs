...
public class GrabColliderController : MonoBehaviour {
	...
	private float throwingPower = 30f;
	...
	private void Grab(Collider col){
		...
		CrateColliderController crateColliderController = col.gameObject.GetComponent<CrateColliderController>();
		if(crateColliderController!=null){
			crateColliderController.isThrown =false;
		}
		...
		col.gameObject.transform.position = U3DUtils.SetVector3D(col.gameObject.transform.position, hero.gameObject.transform.position.x, hero.gameObject.transform.position.y + 2f);
	}
	...
	private void ThrowObject(){
		if(heroController.isHoldingSomething){
			...
			if(itemHolder.gameObject.transform.childCount >0){
				...
				CrateColliderController crateColliderController = child.gameObject.GetComponent<CrateColliderController>();
				if(crateColliderController!=null){
					crateColliderController.isThrown =true;
				}
				...
				if(heroController.isLookingUp){
					...
					if(heroController.isFacingRight){
						child.gameObject.transform.position = new Vector3(child.gameObject.transform.position.x + 2f,child.gameObject.transform.position.y,child.gameObject.transform.position.z);
