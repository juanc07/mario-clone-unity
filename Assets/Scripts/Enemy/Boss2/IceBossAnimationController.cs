...
public class IceBossAnimationController : CharacterAnimationController {
	...
	{
		base.Attack2 ();
		if(modelAnimation.GetClip(ANIMATION_THROW) != null){
			if(!modelAnimation.IsPlaying(ANIMATION_THROW)){
				modelAnimation.Play(ANIMATION_THROW);
			}
		}
	}
